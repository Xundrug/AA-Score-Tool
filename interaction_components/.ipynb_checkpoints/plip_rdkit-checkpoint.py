...
from plip_rdkit.utils import *

from plip_rdkit.detection_rdkit import halogen, pication, water_bridges, metal_complexation
from plip_rdkit.detection_rdkit import hydrophobic_interactions, pistacking, hbonds, saltbridge
from plip_rdkit import config

FEATURE_FILE = "/home/pxl/localhome2/Study/drug_design/BaseFeatures.fdef.jcg"
...
class GetFeatures:
    def __init__(self, mol):
        ...
        self.get_property_atom()
        
    def get_features(self):
        factory = ChemicalFeatures.BuildFeatureFactory(FEATURE_FILE)
        features = factory.GetFeaturesForMol(self.mol)
        return features
    
    def get_property_atom(self):
        donors, acceptors, hydrophobics = (), (), ()
        for f in self.features:
            if f.GetFamily() == "Donor":
                donors += f.GetAtomIds()
            elif f.GetFamily() == "Acceptor":
                acceptors += f.GetAtomIds()
            elif f.GetFamily() in ["Hydrophobe",  "LumpedHydrophobe"]:
                hydrophobics += f.GetAtomIds()
            
        self.donors = list(set(donors))
        self.acceptors = list(set(acceptors))
        self.hydrophobics = list(set(hydrophobics))
        return 
...
class Mol:
    ...
    def hydrophobic_atoms(self):
        ...
        atm = [a for a in self.all_atoms if (a.GetAtomicNum() == 6 and set([natom.GetAtomicNum() for natom in a.GetNeighbors()]).issubset({1, 6}))]
        for atom in atm:
    ...
    def find_rings(self):
        ...
        for ring in rings_atom_idx:
            ...
            if 4 < len(ring) <= 6:
                ...
                if is_aromatic(r_atoms) or res[0] in aromatic_amino or ring_is_planar(self.mol_conf, ring, r_atoms):
                    ring_type = '%s-membered' % len(ring)
                    ring_atms = get_coords(self.mol_conf, [r_atoms[0].GetIdx(), r_atoms[2].GetIdx(), r_atoms[4].GetIdx()])
                    ...
                    rings.append(data(atoms=r_atoms,
                                      ...
                                      obj=ring,
                                      center=centroid(get_coords(self.mol_conf, ring)),
                                      type=ring_type))
    ...
    def find_hal(self):
        ...
        for a in [at for at in self.all_atoms if at.GetAtomicNum() in [8, 7, 16]]:
            n_atoms = [na for na in a.GetNeighbors() if na.GetAtomicNum() in [6, 7, 15, 16]]
            if len(n_atoms) == 1:  # Proximal atom
                o_orig_idx = a.GetIdx()
                y_orig_idx = n_atoms[0].GetIdx()
                a_set.append(data(o=a, o_orig_idx=o_orig_idx, y=n_atoms[0], y_orig_idx=y_orig_idx))
        return a_set
...
class Ligand(Mol):
    def __init__(self, mol, mol_water=None):
        ...
        self.centroid = np.round(self.mol_conf.GetPositions().mean(axis=0), 5).tolist()
        self.max_dist_to_center = max((euclidean3d(self.centroid, get_coord(self.mol_conf, a.GetIdx())) for a in self.all_atoms))
        
        self.water = []
        if mol_water != None:
            data = namedtuple('water', 'oxy oxy_orig_idx')
            for hoh in self.all_water_atoms:
                oxy = None
                if hoh.GetAtomicNum() == 8:
                    oxy = hoh
                if oxy is not None:
                    if euclidean3d(self.centroid, get_coord(self.mol_water_conf, oxy.GetIdx())) < self.max_dist_to_center + config.BS_DIST:
    ...
    def find_hba(self):
        ...
        for atom_idx, atom in zip(atom_idx_set, atm):
            if atom.GetAtomicNum() not in [9, 17, 35, 53]:  # Exclude halogen atoms
                a_orig_idx = atom_idx
                a_orig_atom = atom
    ...
    def find_hbd(self):
        ...
        for donor_idx, donor_atom in zip(donor_idxs, donor_atoms):
            in_ring = False
            if not in_ring:
                for adj_atom in [a for a in donor_atom.GetNeighbors() if a.GetAtomicNum() == 1]:
                    d_orig_idx = donor_idx
                    d_orig_atom = donor_atom
                    donor_pairs.append(data(d=donor_atom, d_orig_atom=d_orig_atom, d_orig_idx=d_orig_idx,
                                            h=adj_atom, type='regular'))

        for carbon in self.hydroph_atoms:
            for adj_atom in [a for a in carbon.atom.GetNeighbors() if a.GetAtomicNum() == 1]:
                d_orig_idx = carbon.atom.GetIdx()
                d_orig_atom = carbon.atom
                donor_pairs.append(data(d=carbon, d_orig_atom=d_orig_atom,
                                        d_orig_idx=d_orig_idx, h=adj_atom, type='weak'))
        donor_pairs = sorted(donor_pairs, key=lambda x: (x.d_orig_idx, x.h.GetIdx()))
        return donor_pairs
    ...
    def find_hal(self):
        ...
        for a in self.all_atoms:
            if self.is_functional_group(a, 'halocarbon'):
                n_atoms = [na for na in a.GetNeighbors() if na.GetAtomicNum() == 6]
                x_orig_idx = a.GetIdx()
                orig_x = x_orig_idx
        ...
        for a in self.all_atoms:
            ...
            if self.is_functional_group(a, 'quartamine'):
                a_set.append(data(atoms=[a, ], orig_atoms=[a_orig, ], atoms_orig_idx=[a_orig_idx, ], type='positive',
                                  center=list(get_coord(self.mol_conf, a.GetIdx())), fgroup='quartamine'))
            elif self.is_functional_group(a, 'tertamine'):
                a_set.append(data(atoms=[a, ], orig_atoms=[a_orig, ], atoms_orig_idx=[a_orig_idx, ], type='positive',
                                  center=list(get_coord(self.mol_conf, a.GetIdx())),
                                  fgroup='tertamine'))
            ...
            if self.is_functional_group(a, 'phosphate'):
                ...
                [a_contributing_orig_idx.append(neighbor.GetIdx()) for neighbor in a_contributing]
                ...
                a_set.append(
                    data(atoms=a_contributing, orig_atoms=orig_contributing, atoms_orig_idx=a_contributing_orig_idx,
                         type='negative',
                         center=list(get_coord(self.mol_conf, a.GetIdx())), fgroup='phosphate'))
            if self.is_functional_group(a, 'sulfonicacid'):
                a_contributing = [a, ]
                a_contributing_orig_idx = [a_orig_idx, ]
                [a_contributing.append(neighbor) for neighbor in a.GetNeighbors() if neighbor.GetAtomicNum() == 8]
                [a_contributing_orig_idx.append(neighbor.GetIdx()) for neighbor in a_contributing]
                orig_contributing = a_contributing_orig_idx
                a_set.append(
                    data(atoms=a_contributing, orig_atoms=orig_contributing, atoms_orig_idx=a_contributing_orig_idx,
                         type='negative',
                         center=list(get_coord(self.mol_conf, a.GetIdx())), fgroup='sulfonicacid'))
                ...
                a_set.append(
                    data(atoms=a_contributing, orig_atoms=orig_contributing, atoms_orig_idx=a_contributing_orig_idx,
                         type='negative',
                         center=get_coord(self.mol_conf, a.GetIdx()), fgroup='sulfate'))
            if self.is_functional_group(a, 'carboxylate'):
                a_contributing = [neighbor for neighbor in a.GetNeighbors() if neighbor.GetAtomicNum() == 8]
                a_contributing_orig_idx = [neighbor.GetIdx() for neighbor in a_contributing]
                orig_contributing = a_contributing_orig_idx
                a_set.append(
                    data(atoms=a_contributing, orig_atoms=orig_contributing, atoms_orig_idx=a_contributing_orig_idx,
                         type='negative',
                         center=centroid([get_coord(self.mol_conf, a.GetIdx()) for a in a_contributing]), fgroup='carboxylate'))
            elif self.is_functional_group(a, 'guanidine'):
                a_contributing = [neighbor for neighbor in a.GetNeighbors() if neighbor.GetAtomicNum() == 7]
                a_contributing_orig_idx = [neighbor.GetIdx() for neighbor in a_contributing]
                orig_contributing = a_contributing_orig_idx
                a_set.append(
                    data(atoms=a_contributing, orig_atoms=orig_contributing, atoms_orig_idx=a_contributing_orig_idx,
                         type='positive',
                         center=get_coord(self.mol_conf, a.GetIdx()), fgroup='guanidine'))
        return a_set
    
    def is_functional_group(self, atom, group):
        """Given a pybel atom, look up if it belongs to a function group"""
        n_atoms = [a_neighbor.GetAtomicNum() for a_neighbor in atom.GetNeighbors() if a_neighbor.GetAtomicNum() != 1]
        ...
        if group in ['sulfonium', 'sulfonicacid', 'sulfate'] and atom.GetAtomicNum() == 16:  # Sulfur
        ...
        if group in ['carboxylate', 'guanidine'] and atom.GetAtomicNum() == 6:  # It's a carbon atom
            if n_atoms.count(8) == 2 and n_atoms.count(6) == 1:  # It's a carboxylate group
                ...
                for nitro in atom.GetNeighbors():
                    nitro_partners.append(len([b_neighbor for b_neighbor in nitro.GetNeighbors()]))
        ...
        if group == 'halocarbon' and atom.GetAtomicNum() in [9, 17, 35, 53]:  # Halogen atoms
            n_atoms = [na for na in atom.GetNeighbors() if na.GetAtomicNum() == 6]
    ...
    def find_metal_binding(self):
        ...
        for oxygen in self.water:
            a_set.append(data(atom=oxygen.oxy, atom_orig_idx=oxygen.oxy_orig_idx, 
                              type='O', fgroup='water', restype="HOH", 
                              resnr=oxygen.oxy.GetPDBResidueInfo().GetResidueNumber(),
                              reschain="W",
                              location='water', orig_atom=oxygen.oxy_orig_idx))
        ...
        for a in self.all_atoms:
            ...
            if a.GetAtomicNum() == 8:  # Oxygen
                if n_atoms_atomicnum.count('1') == 1 and len(n_atoms_atomicnum) == 2:  # Oxygen in alcohol (R-[O]-H)
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='O', 
                                      fgroup='alcohol', restype="l", resnr=1, reschain="L",
                                      location='ligand', orig_atom=a_orig_idx))
                if True in [n.GetIsAromatic() for n in n_atoms] and not a.GetIsAromatic():  # Phenolate oxygen
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='O', 
                                      fgroup='phenolate', restype="l", resnr=1, reschain="L",
                                      location='ligand', orig_atom=a_orig_idx))
            if a.GetAtomicNum() == 6:  # It's a carbon atom
                if n_atoms_atomicnum.count(8) == 2 and n_atoms_atomicnum.count(6) == 1:  # It's a carboxylate group
                    for neighbor in [n for n in n_atoms if n.GetAtomicNum() == 8]:
                        ...
                        a_set.append(data(atom=neighbor, atom_orig_idx=neighbor_orig_idx, type='O',
                                          fgroup='carboxylate', restype="l", resnr=1, reschain="L",
                                          location='ligand', orig_atom=a_orig_idx))
            if a.GetAtomicNum() == 15:  # It's a phosphor atom
                if n_atoms_atomicnum.count(8) >= 3:  # It's a phosphoryl
                    for neighbor in [n for n in n_atoms if n.GetAtomicNum() == 8]:
                        neighbor_orig_idx = neighbor.GetIdx()
                        a_set.append(data(atom=neighbor, atom_orig_idx=neighbor_orig_idx, type='O',
                                          fgroup='phosphoryl', restype="l", resnr=1, reschain="L",
                                          location='ligand', orig_atom=a_orig_idx))
                if n_atoms_atomicnum.count(8) == 2:  # It's another phosphor-containing group #@todo (correct name?)
                    for neighbor in [n for n in n_atoms if n.GetAtomicNum() == 8]:
                        neighbor_orig_idx = neighbor.GetIdx()
                        a_set.append(data(atom=neighbor, atom_orig_idx=neighbor_orig_idx, type='O',
                                          fgroup='phosphor.other', restype="l", resnr=1, reschain="L", 
                                          location='ligand', orig_atom=a_orig_idx))
            if a.GetAtomicNum() == 7:  # It's a nitrogen atom
                if n_atoms_atomicnum.count(6) == 2:  # It's imidazole/pyrrole or similar
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='N', 
                                      fgroup='imidazole/pyrrole', restype="l", resnr=1, reschain="L",
                                      location='ligand', orig_atom=a_orig_idx))
            if a.GetAtomicNum() == 16:  # It's a sulfur atom
                if True in [n.GetIsAromatic() for n in n_atoms] and not a.GetIsAromatic():  # Thiolate
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='S', 
                                      fgroup='thiolate', restype="l", resnr=1, reschain="L",
                                      location='ligand', orig_atom=a_orig_idx))
                if set(n_atoms_atomicnum) == {26}:  # Sulfur in Iron sulfur cluster
                    a_set.append(data(atom=a, atom_orig_idx=a_orig_idx, type='S', 
                                      fgroup='iron-sulfur.cluster', restype="l", resnr=1, reschain="L",
                                      location='ligand', orig_atom=a_orig_idx))
        return a_set
...
class Protein(Mol):
    def __init__(self, mol):
        ...
        self.hbond_acc_atoms = self.find_hba()
        self.hbond_don_atom_pairs = self.find_hbd()
        self.residues = residue_order(self.all_atoms)
        ...
        for a in [a for a in self.all_atoms if a.GetSymbol().upper() in config.METAL_IONS]:
            m_orig_idx = a.GetIdx()
            orig_m = m_orig_idx
    ...
    def find_hbd(self):
        ...
        donors = [a for a in self.all_atoms if is_donor(a)]
        for donor in donors:
            in_ring = False
            if not in_ring:
                for adj_atom in [a for a in donor.GetNeighbors() if a.GetAtomicNum() == 1]:
        ...
        for res in self.residues:
            ...
            if res.residue_name in ("ARG", "HIS", "LYS"):
                for a in res.residue_atoms:
                    if a.GetSymbol() == "N" and is_sidechain(a):
                ...
                if not len(a_contributing) == 0:
                    a_set.append(data(atoms=a_contributing,
                                      ...
                                      type='positive',
                                      center=centroid([get_coord(self.mol_conf, ac.GetIdx()) for ac in a_contributing]),
                                      restype=res.residue_name,
                                      resnr=res.residue_number,
                                      reschain='P'))
            if res.residue_name in ("GLU", "ASP"):
                for a in res.residue_atoms:
                    if a.GetSymbol() == "O" and is_sidechain(a):
                        a_contributing.append(a)
                        a_contributing_orig_idx.append(a.GetIdx())
                if not len(a_contributing) == 0:
                    a_set.append(data(atoms=a_contributing,
        ...
        for res in self.residues:
            ...
            if restype in ("ASP", "GLU", "SER", "THR", "TYR"):
                for a in res.residue_atoms:
                    if a.GetSymbol() == "O" and a.GetPDBResidueInfo().GetName().strip(" ") != "O":
                        ...
                        a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='O', restype=restype,
            ...
            if restype == 'HIS':  # Look for nitrogen here
                for a in res.residue_atoms:
                    if a.GetSymbol() == "N" and a.GetPDBResidueInfo().GetName().strip(" ") != "N":
                        atom_orig_idx = a.GetIdx()
                        a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='N', restype=restype,
                                          resnr=resnr, reschain=reschain,
                                          location='protein.sidechain'))
            if restype == 'CYS':  # Look for sulfur here
                for a in res.residue_atoms:
                    if a.GetSymbol() == "S":
                        atom_orig_idx = a.GetIdx()
                        a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='S', restype=restype,
                                          resnr=resnr, reschain=reschain,
                                          location='protein.sidechain'))
            ...
            for a in res.residue_atoms:  # All main chain oxygens
                if a.GetSymbol() == "O" and a.GetPDBResidueInfo().GetName().strip(" ") == "O":
                    atom_orig_idx = a.GetIdx()
                    a_set.append(data(atom=a, atom_orig_idx=atom_orig_idx, type='O', restype=res.residue_name,
                                      resnr=res.residue_number, reschain='',
...
class PLInteraction:
    ...
    def __init__(self, lig_obj, bs_obj, pdbid):
        ...
        self.saltbridge_lneg = saltbridge(self.protein.get_pos_charged(), self.ligand.get_neg_charged(), True)
        self.saltbridge_pneg = saltbridge(self.ligand.get_pos_charged(), self.protein.get_neg_charged(), False)
        ...
        self.all_pi_cation_laro = pication(self.ligand.rings, self.protein.get_pos_charged(), True)
        self.pication_paro = pication(self.protein.rings, self.ligand.get_pos_charged(), False)
        ...
        self.halogen_bonds = halogen(self.protein.halogenbond_acc, self.ligand.halogenbond_don)
        self.water_bridges = water_bridges(self.protein.get_hba(), self.ligand.get_hba(),
                                           self.protein.get_hbd(), self.ligand.get_hbd(),
                                           self.ligand.water)

        self.water_bridges = self.refine_water_bridges(self.water_bridges, self.hbonds_ldon, self.hbonds_pdon)

        self.metal_complexes = metal_complexation(self.protein.metals, self.ligand.metal_binding,
        ...
        self.unpaired_hba_orig_idx = [atom.GetIdx() for atom in self.unpaired_hba]
        self.unpaired_hbd_orig_idx = [atom.GetIdx() for atom in self.unpaired_hbd]
        self.unpaired_hal_orig_idx = [atom.GetIdx() for atom in self.unpaired_hal]
    ...
    def find_unpaired_ligand(self):
        ...
        involved_atoms = [hbond.a.GetIdx() for hbond in self.hbonds_pdon] + [hbond.d.GetIdx() for hbond in self.hbonds_ldon]
        [[involved_atoms.append(atom.GetIdx()) for atom in sb.negative.atoms] for sb in self.saltbridge_lneg]
        [[involved_atoms.append(atom.GetIdx()) for atom in sb.positive.atoms] for sb in self.saltbridge_pneg]
        [involved_atoms.append(wb.a.GetIdx()) for wb in self.water_bridges if wb.protisdon]
        [involved_atoms.append(wb.d.GetIdx()) for wb in self.water_bridges if not wb.protisdon]
        [involved_atoms.append(mcomplex.target.atom.GetIdx()) for mcomplex in self.metal_complexes
        ...
        [involved_atoms.append(atom.don.x.GetIdx()) for atom in self.halogen_bonds]
    ...
    def refine_hydrophobic(self, all_h, pistacks):
        ...
        for pistack, h in itertools.product(pistacks, all_h):
            ...
            brs, lrs = [p1.GetIdx() for p1 in pistack.proteinring.atoms], [p2.GetIdx() for p2 in pistack.ligandring.atoms]
            if h1 in brs and h2 in lrs:
                sel[(h1, h2)] = "EXCLUDE"
        hydroph = [h for h in all_h if not (h.bsatom.GetIdx(), h.ligatom.GetIdx()) in sel]
        sel2 = {}
    ...
    def refine_hbonds_ldon(self, all_hbonds, salt_lneg, salt_pneg):
        ...
        for hbond in all_hbonds:
            ...
            for salt in salt_pneg:
                protidx, ligidx = [at.GetIdx() for at in salt.negative.atoms], [at.GetIdx() for at in salt.positive.atoms]
                if hbond.d.GetIdx() in ligidx and hbond.a.GetIdx() in protidx:
                    i_set[hbond] = True
            for salt in salt_lneg:
                protidx, ligidx = [at.GetIdx() for at in salt.positive.atoms], [at.GetIdx() for at in salt.negative.atoms]
                if hbond.d.GetIdx() in ligidx and hbond.a.GetIdx() in protidx:
                    i_set[hbond] = True
    ...
    def refine_water_bridges(self, wbridges, hbonds_ldon, hbonds_pdon):
        ...
        donor_atoms_hbonds = [hb.d.GetIdx() for hb in hbonds_ldon + hbonds_pdon]
        wb_dict = {}
        wb_dict2 = {}
        omega = 110.0

        # Just one hydrogen bond per donor atom
        for wbridge in [wb for wb in wbridges if wb.d.GetIdx() not in donor_atoms_hbonds]:
            if (wbridge.water.GetIdx(), wbridge.a.GetIdx()) not in wb_dict:
                wb_dict[(wbridge.water.GetIdx(), wbridge.a.GetIdx())] = wbridge
            else:
                if abs(omega - wb_dict[(wbridge.water.GetIdx(), wbridge.a.GetIdx())].w_angle) < abs(omega - wbridge.w_angle):
                    wb_dict[(wbridge.water.GetIdx(), wbridge.a.GetIdx())] = wbridge
...
def func(interaction):
    print("---------------------------------------    hydrophobic   -------------------------------------------------")
    print("hydrophobic", len(interaction.hydrophobic_contacts))
    print("restype", "resnr", "pidx", "distance", "lidx")
    for pl in interaction.hydrophobic_contacts:
        print(pl.restype, pl.resnr, pl.bsatom_orig_idx, round(pl.distance, 3), pl.ligatom_orig_idx)
    
    print("---------------------------------------    hbonds    -------------------------------------------------")
    print("hbonds", len(interaction.hbonds_ldon), len(interaction.hbonds_pdon))
    print("restype", "resnr", "distance_ah", "distance_ad", "angle", "atype", "dtype", "pdonor")
    for pl in interaction.hbonds_ldon:
        print(pl.restype, pl.resnr, round(pl.distance_ah, 3), round(pl.distance_ad, 3), round(pl.angle, 3), pl.atype, pl.dtype, False)
    for pl in interaction.hbonds_pdon:
        print(pl.restype, pl.resnr, round(pl.distance_ah, 3), round(pl.distance_ad, 3), round(pl.angle, 3), pl.atype, pl.dtype, True)
    
    print("---------------------------------------    waterbridge    -------------------------------------------------")
    print("waterbridge", len(interaction.water_bridges))
    print("restype", "resnr", "distance_aw", "distance_dw", "d_angle", "w_angle")
    for wb in interaction.water_bridges:
        print(wb.restype, wb.resnr, round(wb.distance_aw, 3), round(wb.distance_dw, 3), round(wb.d_angle, 3), round(wb.w_angle, 3),  whichatomname(wb.a),wb.d.GetSymbol(), wb.d.GetIdx())
    
    print("---------------------------------------    pi stacking    -------------------------------------------------")
    print("pi-stacking", len(interaction.pistacking))
    print("restype", "resnr", "distance", "angle", "offset")
    for pis in interaction.pistacking:
        print(pis.restype, pis.resnr, round(pis.distance, 3), round(pis.angle, 3), round(pis.offset, 3))
    
    print("---------------------------------------    pi-cation    -------------------------------------------------")
    print("pi-cation", len(interaction.pication_paro), len(interaction.pication_laro))
    print("restype", "resnr", "distance", "offset", "pcharge")
    for pic in interaction.pication_paro:
        print(pic.restype, pic.resnr, round(pic.distance, 3), round(pic.offset,3), False)
    for pic in interaction.pication_laro:
        print(pic.restype, pic.resnr, round(pic.distance, 3), round(pic.offset,3), True)
    
    print("---------------------------------------    saltbridge    -------------------------------------------------")
    print("saltbridge", len(interaction.saltbridge_lneg), len(interaction.saltbridge_pneg))
    print("restype", "resnr", "distance", "protein positive", "group")
    for sa in interaction.saltbridge_lneg:
        print(sa.restype, sa.resnr, round(sa.distance, 3), True, sa.negative.fgroup)
    for sa in interaction.saltbridge_pneg:
        print(sa.restype, sa.resnr, round(sa.distance, 3), False, sa.positive.fgroup)


def get_interaction(protein_file, ligand_file, water_file=None, pdbid=None):
    """
    protein_file: .pdb
    ligand_file: .sdf
    water_file: .pdb
    """
    mol_lig = next(Chem.SupplierFromFilename(ligand_file, removeHs=False))
    mol_protein = Chem.MolFromPDBFile(protein_file, removeHs=False)
    if water_file != None:
        mol_water = Chem.MolFromPDBFile(water_file, removeHs=False)
        ligand = Ligand(mol_lig, mol_water)
    else:
        ligand = Ligand(mol_lig)
    protein = Protein(mol_protein)
    interaction = PLInteraction(ligand, protein, pdbid)
    return interaction

if __name__=="__mian__":
    pdbid = "4mww"
    mol_lig = next(Chem.SupplierFromFilename("/home/pxl/localhome2/Study/drug_design/test/data/{}/lig1.sdf".format(pdbid), removeHs=False))
    mol_water = Chem.MolFromPDBFile("/home/pxl/localhome2/Study/drug_design/test/data/{}/waters1.pdb".format(pdbid), removeHs=False)
    mol_protein = Chem.MolFromPDBFile("/home/pxl/localhome2/Study/drug_design/test/data/{}/pro1.pdb".format(pdbid), removeHs=False)
    ligand = Ligand(mol_lig, mol_water)
    protein = Protein(mol_protein)
    print(len(ligand.get_hbd()))
    for ld in ligand.get_hbd():
        print(ld.d.GetSymbol(), ld.d.GetIdx())
    print(len(ligand.get_hba()))
    for la in ligand.get_hba():
        print(la.a.GetSymbol(), la.a.GetIdx())
