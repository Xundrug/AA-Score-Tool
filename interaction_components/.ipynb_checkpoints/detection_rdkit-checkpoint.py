...
from plip_rdkit.utils import vecangle, vector, euclidean3d, projection, is_sidechain
from plip_rdkit.utils import whichresnumber, whichrestype, whichchain
...
def hydrophobic_interactions(atom_set_a, atom_set_b):
    ...
    for a, b in itertools.product(atom_set_a, atom_set_b):
        if a.orig_idx == b.orig_idx:
            continue
        e = euclidean3d(coords(a.atom), coords(b.atom))
        if not config.MIN_DIST < e < config.HYDROPH_DIST_MAX:
            continue
        restype, resnr, reschain = whichrestype(a.atom), whichresnumber(a.atom), "P"
...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        dist_ah = euclidean3d(coords(acc.a), coords(don.h))
        dist_ad = euclidean3d(coords(acc.a), coords(don.d))
        if not config.MIN_DIST < dist_ad < config.HBOND_DIST_MAX:
            continue
        vec1, vec2 = vector(coords(don.h), coords(don.d)), vector(coords(don.h), coords(acc.a))
        ...
        if config.INTRA is not None and whichresnumber(don.d) == whichresnumber(acc.a):
            continue
        if config.INTRA is not None and is_sidechain(protatom):
            continue
        contact = data(a=acc.a, a_orig_idx=acc.a_orig_idx, d=don.d, d_orig_idx=don.d_orig_idx, h=don.h,
                       ...
                       atype=acc.a.GetSymbol(), dtype=don.d.GetSymbol())
        pairings.append(contact)
    return filter_contacts(pairings)
...
def pistacking(rings_bs, rings_lig):
    ...
    for r, l in itertools.product(rings_bs, rings_lig):
        ...
        # RECEPTOR DATA
        resnr, restype, reschain = whichresnumber(r.atoms[0]), whichrestype(r.atoms[0]), "P"
    ...
    for ring in rings:
        ...
        for p in pos_charged:
            ...
            if type(p).__name__ == 'lcharge' and p.fgroup == 'tertamine':
                ...
                n_atoms = [a_neighbor for a_neighbor in p.atoms[0].GetNeighbors()]
                n_atoms_coords = [coords(a) for a in n_atoms]
...
def halogen(acceptor, donor):
    ...
    for acc, don in itertools.product(acceptor, donor):
        dist = euclidean3d(coords(acc.o), coords(don.x))
        if not config.MIN_DIST < dist < config.HALOGEN_DIST_MAX:
            continue
        vec1, vec2 = vector(coords(acc.o), coords(acc.y)), vector(coords(acc.o), coords(don.x))
        vec3, vec4 = vector(coords(don.x), coords(acc.o)), vector(coords(don.x), coords(don.c))
        ...
        if not config.HALOGEN_DON_ANGLE - config.HALOGEN_ANGLE_DEV < don_angle \
            ...
            continue
        restype, reschain, resnr = whichrestype(acc.o), "P", whichresnumber(acc.o)
        ...
        contact = data(acc=acc, acc_orig_idx=acc.o_orig_idx, don=don, don_orig_idx=don.x_orig_idx,
                       ...
                       reschain=reschain, restype_l=restype_l,
                       reschain_l=reschain_l, resnr_l=resnr_l, donortype=don.x.GetSymbol(), acctype=acc.o.GetSymbol(),
...
def water_bridges(bs_hba, lig_hba, bs_hbd, lig_hbd, water):
    ...
    for w in water:
        for acc1 in lig_hba:
            dist = euclidean3d(coords(acc1.a), coords(w.oxy))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST:
                lig_aw.append((acc1, w, dist))
        for acc2 in bs_hba:
            dist = euclidean3d(coords(acc2.a), coords(w.oxy))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST:
                prot_aw.append((acc2, w, dist))
        for don1 in lig_hbd:
            dist = euclidean3d(coords(don1.d), coords(w.oxy))
            d_angle = vecangle(vector(coords(don1.h), coords(don1.d)), vector(coords(don1.h), coords(w.oxy)))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
                lig_dw.append((don1, w, dist, d_angle))
        for don2 in bs_hbd:
            dist = euclidean3d(coords(don2.d), coords(w.oxy))
            d_angle = vecangle(vector(coords(don2.h), coords(don2.d)), vector(coords(don2.h), coords(w.oxy)))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
    ...
    for l, p in itertools.product(lig_aw, prot_hw):
        ...
        if not wl.oxy.GetIdx() == wd.oxy.GetIdx():
            continue
        # Same water molecule and angle within omega
        w_angle = vecangle(vector(coords(acc.a), coords(wl.oxy)), vector(coords(wl.oxy), coords(don.h)))
        if not config.WATER_BRIDGE_OMEGA_MIN < w_angle < config.WATER_BRIDGE_OMEGA_MAX:
            continue
        resnr, reschain, restype = whichresnumber(don.d), "P", whichrestype(don.d)
        resnr_l, reschain_l, restype_l = 1, "L", "Lig"
        contact = data(a=acc.a, a_orig_idx=acc.a_orig_idx, atype=acc.a.GetSymbol(), d=don.d, d_orig_idx=don.d_orig_idx,
                       dtype=don.d.GetSymbol(), h=don.h, water=wl.oxy, water_orig_idx=wl.oxy_orig_idx,
    ...
    for p, l in itertools.product(prot_aw, lig_dw):
        ...
        if not config.WATER_BRIDGE_OMEGA_MIN < w_angle < config.WATER_BRIDGE_OMEGA_MAX:
            continue
        resnr, reschain, restype = whichresnumber(acc.a), "P", whichrestype(acc.a)
        resnr_l, reschain_l, restype_l = 1, "L", "Lig"
        contact = data(a=acc.a, a_orig_idx=acc.a_orig_idx, atype=acc.a.GetSymbol(), d=don.d, d_orig_idx=don.d_orig_idx,
                       dtype=don.d.GetSymbol(), h=don.h, water=wl.oxy, water_orig_idx=wl.oxy_orig_idx,
...
def metal_complexation(metals, metal_binding_lig, metal_binding_bs):
    ...
    for metal, target in itertools.product(metals, metal_binding_lig + metal_binding_bs):
        distance = euclidean3d(coords(metal.m), coords(target.atom))
    ...
    for cnum, metal in enumerate(pairings_dict):
        ...
        for contact_pair in contact_pairs:
            ...
            vectors_dict[target.atom.GetIdx()].append(vector(coords(metal), coords(target.atom)))
        ...
        if not only_water:  # No complex if just with water as targets
            for contact_pair in contact_pairs:
                ...
                if target.atom.GetIdx() not in excluded:
                    ...
                    restype_l, reschain_l, resnr_l = "Lig", "L", 1
                    contact = data(metal=metal, metal_orig_idx=metal_to_id[metal], metal_type=metal.GetSymbol(),
