...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        dist_ah = euclidean3d(acc.coords, don.h_coords)
        dist_ad = euclidean3d(acc.coords, don.d_coords)
        if not config.MIN_DIST < dist_ad < config.HBOND_DIST_MAX:
            continue
        vec1, vec2 = vector(
            don.h_coords, don.d_coords), vector(
            don.h_coords, acc.coords)
        v = vecangle(vec1, vec2)
        if not v > config.HBOND_DON_ANGLE_MIN:
            continue
        protatom = don.d if protisdon else acc.a
        ligatom = don.d if not protisdon else acc.a
        is_sidechain_hbond = is_sidechain(protatom)
        ...
        contact = data(
            ...
            atype=acc.a.GetSymbol(),
            dtype=don.d.GetSymbol())
        pairings.append(contact)
    return filter_contacts(pairings)
...
def pistacking(rings_bs, rings_lig):
    ...
    for r, l in itertools.product(rings_bs, rings_lig):
        ...
        # RING CENTER OFFSET CALCULATION (project each ring center into the
        # other ring)
        proj1 = projection(l.normal, l.center, r.center)
        proj2 = projection(r.normal, r.center, l.center)
        offset = min(
            euclidean3d(
                proj1, l.center), euclidean3d(
                proj2, r.center))
    ...
    for ring in rings:
        ...
        for p in pos_charged:
            ...
            restype = whichrestype(
                p.atoms[0]) if protcharged else whichrestype(
                ring.atoms[0])
            restype_l = "Lig"
            reschain = whichchain(
                p.atoms[0]) if protcharged else whichchain(
                ring.atoms[0])
            reschain_l = 'L'
            contact = data(
                ring=ring,
...
def saltbridge(poscenter, negcenter, protispos):
    ...
    for pc, nc in itertools.product(poscenter, negcenter):
        if not config.MIN_DIST < euclidean3d(
                pc.center, nc.center) < config.SALTBRIDGE_DIST_MAX:
            continue
...
def halogen(acceptor, donor):
    ...
    for acc, don in itertools.product(acceptor, donor):
        dist = euclidean3d(acc.o_coords, don.x_coords)
        if not config.MIN_DIST < dist < config.HALOGEN_DIST_MAX:
            continue
        vec1, vec2 = vector(
            acc.o_coords, acc.y_coords), vector(
            acc.o_coords, don.x_coords)
        vec3, vec4 = vector(
            don.x_coords, acc.o_coords), vector(
            don.x_coords, don.c_coords)
        acc_angle, don_angle = vecangle(vec1, vec2), vecangle(vec3, vec4)
        is_sidechain_hal = is_sidechain(acc.o)  # Check if sidechain atom
        ...
        if not config.HALOGEN_DON_ANGLE - config.HALOGEN_ANGLE_DEV < don_angle \
            ...
            continue
        restype, reschain, resnr = whichrestype(
            acc.o), "P", whichresnumber(acc.o)
        restype_l, reschain_l, resnr_l = "Lig", "L", 1
        contact = data(
            acc=acc,
            ...
            reschain_l=reschain_l,
            resnr_l=resnr_l,
            donortype=don.x.GetSymbol(),
            acctype=acc.o.GetSymbol(),
...
def water_bridges(bs_hba, lig_hba, bs_hbd, lig_hbd, water):
    ...
    data = namedtuple(
        'waterbridge', 'a a_orig_idx atype d d_orig_idx dtype h water water_orig_idx distance_aw '
    ...
    for w in water:
        for acc1 in lig_hba:
            dist = euclidean3d(acc1.coords, w.oxy_coords)
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST:
                lig_aw.append((acc1, w, dist))
        for acc2 in bs_hba:
            dist = euclidean3d(acc2.coords, w.oxy_coords)
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST:
                prot_aw.append((acc2, w, dist))
        for don1 in lig_hbd:
            dist = euclidean3d(don1.d_coords, w.oxy_coords)
            d_angle = vecangle(
                vector(
                    don1.h_coords, don1.d_coords), vector(
                    don1.h_coords, w.oxy_coords))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
                lig_dw.append((don1, w, dist, d_angle))
        for don2 in bs_hbd:
            dist = euclidean3d(don2.d_coords, w.oxy_coords)
            d_angle = vecangle(
                vector(
                    don2.h_coords, don2.d_coords), vector(
                    don2.h_coords, w.oxy_coords))
            if config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST \
                    and d_angle > config.WATER_BRIDGE_THETA_MIN:
    ...
    for l, p in itertools.product(lig_aw, prot_hw):
        ...
        w_angle = vecangle(
            vector(
                acc.coords, wl.oxy_coords), vector(
                wl.oxy_coords, don.h_coords))
        if not config.WATER_BRIDGE_OMEGA_MIN < w_angle < config.WATER_BRIDGE_OMEGA_MAX:
            continue
        resnr, reschain, restype = whichresnumber(
            don.d), whichchain(
            don.d), whichrestype(
            don.d)
        resnr_l, reschain_l, restype_l = 1, "L", "Lig"
        contact = data(
            a=acc.a,
            a_orig_idx=acc.a_orig_idx,
            atype=acc.a.GetSymbol(),
...
def metal_complexation(metals, metal_binding_lig, metal_binding_bs):
    ...
    for metal, target in itertools.product(
        ...
        distance = euclidean3d(metal.m_coords, target.coords)
    ...
    for cnum, metal in enumerate(pairings_dict):
        ...
        for contact_pair in contact_pairs:
            ...
            vectors_dict[target.atom.GetIdx()].append(
                vector(get_atom_coords(metal), target.coords))
        ...
        ideal_angles = {'linear': [[180.0]] *
                        2, 'trigonal.planar': [[120.0, 120.0]] *
                        3, 'trigonal.pyramidal': [[109.5, 109.5]] *
                        3, 'tetrahedral': [[109.5, 109.5, 109.5, 109.5]] *
                        4, 'square.planar': [[90.0, 90.0, 90.0, 90.0]] *
                        4, 'trigonal.bipyramidal': [[120.0, 120.0, 90.0, 90.0]] *
                        3 +
                        [[90.0, 90.0, 90.0, 180.0]] *
                        2, 'square.pyramidal': [[90.0, 90.0, 90.0, 180.0]] *
                        4 +
                        [[90.0, 90.0, 90.0, 90.0]], 'octahedral': [[90.0, 90.0, 90.0, 90.0, 180.0]] *
                        6}
        angles_dict = {}
        ...
        for target in vectors_dict:
            ...
            angles = [
                vecangle(
                    pair[0],
                    pair[1]) for pair in itertools.product(
                    cur_vector,
                    other_vectors)]
        ...
        if num_targets == 1:
            ...
            for coo in sorted(
                ...
                for geometry in geometries:
                    ...
                    signature = ideal_angles[geometry]
                    geometry_total = 0
                    ...
                    for subsignature in signature:  # Ideal angles from one perspective
                        ...
                        for k, target in enumerate(angles_dict):
                            if target not in used_up_targets:
                                # Observed angles from perspective of one
                                # target
                                observed_angles = angles_dict[target]
                                single_target_scores = []
                                used_up_observed_angles = []
                                for i, ideal_angle in enumerate(subsignature):
                                    # For each angle in the signature, find the
                                    # best-matching observed angle
                                    best_match = None
                                    best_match_diff = 999
                                ...
                                # Tot. score targ/sig
                                target_total = sum(
                                    [x ** 2 for x in single_target_scores]) ** 0.5
                    ...
                    # Record the targets not used for excluding them when
                    # deciding for a final geometry
                    [not_used.append(
                        target) for target in angles_dict if target not in used_up_targets]
                    all_total.append(
                        gdata(
                            geometry=geometry,
                            rms=geometry_total,
        ...
        # Check if the difference between the RMS to the next best solution is
        # not larger than 0.5
